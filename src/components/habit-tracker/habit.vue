<template>
  <div :class="[isMobile() ? 'container' : '']">
    <h2 style="padding: 10px; text-align:center">
      Il mio habit tracker settimanale <span style="font-size: 30px">🏡</span>
    </h2>
    ...
    <p style="text-align:center">
      ...
      studiare, disegnare, meditare, ecc.<br />Inserisci tutto ciò che vuoi, e
      visualizza le statistiche settimanali nel grafico sottostante, cliccando
    ...
    <div id="habits" class="table-responsive">
      <table class="table">
        <thead>
          <tr>
            <th>Inserisci le abitudini che vuoi coltivare 🌱</th>
            <th>Lunedì</th>
            <th>Martedì</th>
            <th>Mercoledì</th>
            <th>Giovedì</th>
          ...
          <tr v-for="(days, i) in weekDaysByStatus" :key="i">
            <td>
              <input
                ...
                @change="updateWeekDay(weekDays[i])"
              />
            </td>
            ...
            <th>
              <button
                ...
                @click="
                  weekDays[i].isActiveMonday = !weekDays[i].isActiveMonday;
                  updateWeekDay(weekDays[i]);
                "
              >
            ...
            <td>
              <button
                ...
                @click="
                  weekDays[i].isActiveFriday = !weekDays[i].isActiveFriday;
                  updateWeekDay(weekDays[i]);
                "
              >
            ...
            <td>
              <button
                ...
                @click="
                  weekDays[i].isActiveSunday = !weekDays[i].isActiveSunday;
                  updateWeekDay(weekDays[i]);
                "
              >
            ...
            <td>
              <button
                class="btn btn-lg"
                type="button"
                v-on:click="deleteRow(weekDays[i].id)"
                aria-label="Delete"
                title="Delete"
      ...
      <div id="chart">
        <p style="text-align:center">
          Queste sono le statistiche delle tue abitudini:
        </p>
        <br />
...
<script>
import { getHabits } from "./utils.js";
...
export default {
  ...
  methods: {
    ...
    addNewRow() {
      fetch(process.env.VUE_APP_API_URL + "api/user/createHabit", {
        method: "POST",
        mode: "cors",
        headers: {
        ...
        .then(res => {
          ...
          getHabits(this.user.id).then(weekDays => (this.weekDays = weekDays));
        })
        .catch(err => {
    ...
    updateWeekDay(habit) {
      fetch(process.env.VUE_APP_API_URL + "api/user/updateHabit", {
        method: "POST",
        mode: "cors",
        headers: {
          "Content-Type": "application/json",
    ...
    deleteRow(id) {
      fetch(process.env.VUE_APP_API_URL + "api/user/removeHabit", {
        method: "POST",
        mode: "cors",
        headers: {
          "Content-Type": "application/json",
        ...
        .then(res => {
          ...
          getHabits(this.user.id).then(weekDays => {
    ...
    union(A, B) {
      var obj = {};
      for (var i = A.length - 1; i >= 0; --i) obj[A[i]] = A[i];
      for (var i = B.length - 1; i >= 0; --i) obj[B[i]] = B[i];
      var res = [];
      var app = [];
      for (var k in obj) {
  ...
  mounted() {
    if (this.$props.user) {
      getHabits(this.$props.user.id).then(
        weekDays => (this.weekDays = weekDays)
      );
    }
  },
  watch: {
    user(u) {
      getHabits(u.id).then(weekDays => (this.weekDays = weekDays));
    },
  },
};
</script>
